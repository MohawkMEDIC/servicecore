...
CREATE INDEX QRY_ID_IDX ON QRY_TBL(QRY_ID);
...
(
	...
	ENT_ID		VARCHAR(128) NOT NULL, -- THE ENTERPRISE IDENTIFIER (THE QUERY READER SHOULD UNDERSTAND WHAT THIS IS)
	VRSN_ID		VARCHAR(128), -- THE VERSION ID
...
(
	QRY_ID_IN	IN VARCHAR(128),
	RSLT_ENT_ID_IN	IN VARCHAR(128),
	RSLT_VRSN_ID_IN	IN VARCHAR(120)
) RETURNS VOID AS 
$$
BEGIN
...
(
	QRY_ID_IN	IN TEXT,
	RSLT_ENT_ID_IN	IN TEXT
) RETURNS VOID AS 
$$
DECLARE
	RSLT_ENT_ID_ARR VARCHAR[][];
BEGIN

	
	RSLT_ENT_ID_ARR := RSLT_ENT_ID_IN;
	FOR RSLT IN ARRAY_LOWER(RSLT_ENT_ID_ARR, 1) .. ARRAY_UPPER(RSLT_ENT_ID_ARR, 1) LOOP
	...
	ELSE
		...
		IF STR_IN IS NOT NULL THEN
			SELECT RSLT_ID INTO frst_rslt FROM
				...
				LIMIT 1;
		ELSE
			SELECT RSLT_ID INTO frst_rslt FROM QRY_RSLT_TBL WHERE QRY_ID = QRY_ID_IN AND RTR_UTC IS NULL ORDER BY RSLT_ID LIMIT 1;
				
		END IF;
		
		UPDATE QRY_RSLT_TBL SET RTR_UTC = CURRENT_TIMESTAMP WHERE RSLT_ID IN (SELECT RSLT_ID FROM QRY_RSLT_TBL 
