...
(
	QRY_ID		VARCHAR(48) NOT NULL, -- THE IDENTIFIER OF THE QUERY
	QRY_QTY		DECIMAL(4,0) NOT NULL, -- THE TOTAL QUANTITY OF RESULTS IN THE QUERY
	QRY_DOMAIN	TEXT NOT NULL, -- THE DOMAIN OF THE QUERY
...
CREATE UNIQUE INDEX QRY_TBL_IDX ON QRY_TBL(QRY_ID, QRY_DOMAIN);
...
(
	...
	QRY_ID		VARCHAR(48) NOT NULL, -- THE ID OF THE QUERY TO WHICH THIS RESULT BELONGS
...
(
	QRY_ID_IN	IN VARCHAR(48),
	QRY_RSLT_CNT_IN	IN DECIMAL(4,0),
	QRY_DMN_IN	IN TEXT
) RETURNS VOID AS
$$
BEGIN
	INSERT INTO QRY_TBL (QRY_ID, QRY_QTY, QRY_DOMAIN) VALUES
		(QRY_ID_IN, QRY_RSLT_CNT_IN, QRY_DMN_IN);
	RETURN;
END
$$
LANGUAGE PLPGSQL;
...
CREATE OR REPLACE FUNCTION GET_QRY_DMN
(
	QRY_ID_IN	IN VARCHAR(48)
) RETURNS TEXT AS
$$
BEGIN
	RETURN (SELECT QRY_DOMAIN FROM QRY_TBL WHERE QRY_ID = QRY_ID_IN);
END;
$$ LANGUAGE plpgsql;